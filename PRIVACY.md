...
Durepo accesses only repository or project folders that you explicitly select through the macOS folder picker. This can include file contents, file names and paths, timestamps, permissions, extended attributes, ACLs, symbolic links, Git metadata in `.git`, uncommitted changes, and untracked files. Durepo stores a macOS security-scoped bookmark so the background agent can continue protecting the selected folder. It checks local disk capacity to avoid starting a snapshot when space is insufficient.
...
Snapshots, content-addressed objects, repository bookmarks, and settings are stored locally in Durepo's sandboxed App Group container. Integrity reports are generated locally and are written only to a location you explicitly choose when exporting one. Diagnostic messages use Apple's unified logging system and are designed not to record file contents; sensitive paths are marked private where practical.
...
Durepoは、macOSのフォルダ選択画面で利用者が明示的に選択したリポジトリまたはプロジェクトフォルダだけにアクセスします。対象には、ファイル内容、ファイル名とパス、日時、権限、拡張属性、ACL、シンボリックリンク、`.git`内のGitメタデータ、未コミット変更、未追跡ファイルが含まれる場合があります。バックグラウンドエージェントが継続して保護できるよう、macOSのsecurity-scoped bookmarkを保存します。空き容量不足時にスナップショットを開始しないため、ローカルディスク容量を確認します。
...
スナップショット、CASオブジェクト、リポジトリのbookmark、設定は、DurepoのSandbox化されたApp Group containerにローカル保存されます。整合性レポートはローカルで生成され、書き出す場合にのみ利用者が明示的に選択した場所へ保存されます。診断メッセージにはAppleの統合ログを使用し、ファイル内容を記録せず、可能な箇所では機密性のあるパスをprivateとして扱います。
