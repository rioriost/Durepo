...
smoke test段階では、App Store適合構成を先に固定し、安全な最小経路を実装します。FSEvents path journalとSQLite current-entry indexによる差分処理、APFS file clone最適化までは実装済みです。GCと元位置復元は、壊れ方が大きいため後続フェーズへ分離します。
...
| 脅威モデル | AIエージェントが同じユーザーかつ広いアクセス権を持つ場合、Durepoの保存領域も削除できる | 対象は直近の破壊的操作からのローカル復旧。ownCloud/iCloud、外付け、remote pushによる長期バックアップは製品目的外 |
...
| TOCTOU | ハッシュ中にファイルが変更・置換されると、hashと内容/metadataが不整合になる | 同一APFS volumeでは`O_NOFOLLOW`でopenしたfdから`fclonefileat`し、原子的に固定したcloneをhashする。fallback copyは前後の`fstat`でdevice/inode/size/mtime/ctimeを比較し、変化時は1回retry |
...
| 読み取り負荷 | ファイルごとのTask乱立はSSDとメモリを圧迫する | utility priorityのbounded task groupを最大4に制限。clone非対応時は4 MiB以下をmemory、超過分を1 MiB streamingとし、OSLog/signpostで実測する |
