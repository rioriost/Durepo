...
| 項目 | 問題 | 決定/対策 |
|---|---|---|
| App Sandbox | Mac App Storeでは必須。Sandbox無効案は審査前提と矛盾する | GUI/AgentともSandbox化し、ユーザー選択read-write bookmarkとApp Groupだけを許可 |
...
- privacy manifestは現時点で使用APIにrequired-reason対象が追加される可能性をApp Store upload前にXcodeのPrivacy Reportで再確認する。
...
| 機能 | 評価 | 理由 |
|---|---|---|
| ユーザー選択rootの継続監視 | 条件付きで実装可能 | bookmarkをAgentへ共有し、アクセスを保持する実機検証が必要 |
